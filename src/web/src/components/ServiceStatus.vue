            ...
            <div v-if="service.status != null">
              ...
              <div class="statusTable">
                ...
                <div class="statusTableRow">
                  ...
                  <div class="statusTableCell">
                    {{ service.status.name }} v{{ service.status.version }}
                  </div>
                </div>
                <div class="statusTableRow">
                  <div class="statusTableCell">ОС</div>
                  <div class="statusTableCell">
                    {{ service.status.machine_name }} ({{
                      service.status.os_version
                    }}) {{ service.status.processor_count }}
                    {{ get_declination(service.status.processor_count) }}
                  </div>
                </div>
                <div class="statusTableRow">
                  ...
                  <div class="statusTableCell">
                    <div class="memoryUsage">
                      <el-progress
                        type="dashboard"
                        :percentage="service.status.used_memory_percentage"
                        :color="colors"
                      ></el-progress>
                      Использовано {{ service.status.used_memory }} из
                      {{ service.status.total_memory }}. Свободно
                      {{ service.status.free_memory }}
                    </div>
                  </div>
                </div>
                <div class="statusTableRow">
                  ...
                  <div class="statusTableCell">
                    {{ service.status.start_time }} (работает
                    {{ service.status.working_time }})
                  </div>
                </div>
                <div class="statusTableRow">
                  <div class="statusTableCell">Данные обновлены в</div>
                  <div class="statusTableCell">
                    {{ service.status.creation_date }}
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    load_service_status: function () {
      axios
        .post(api_url + "/orchestrator/service_status", {
