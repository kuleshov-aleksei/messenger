...
--
-- Host: encamy.keenetic.pro    Database: dcsm
-- ------------------------------------------------------
...
--
-- Dumping routines for database 'dcsm'
--
/*!50003 DROP PROCEDURE IF EXISTS `p_is_user_authorized` */;
...
DELIMITER ;;
CREATE DEFINER=`dcsm`@`%` PROCEDURE `p_is_user_authorized`(
IN user_id INTEGER,
...
/*!50013 DEFINER=`dcsm`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `session_view` AS select `session`.`session_id` AS `session_id`,`session`.`user_id` AS `user_id`,`session`.`device_name` AS `device_name`,`session`.`token` AS `token`,((now() < `session`.`expires_at`) and (`session`.`sign_out` = 0)) AS `available` from `session` */;
...
-- Dump completed on 2020-10-05 21:15:52
